...
import cn.bittonet.news.utils.StringUtils;
...
public class StateView extends View {
    ...
    public StateView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (mEmptyResource == 0) {
            mEmptyResource = R.layout.empty;
        }
        if (mRetryResource == 0) {
            mRetryResource = R.layout.error;
        }
    ...
    public View showEmpty(String text) {
        return showEmpty(text, -1);
    }

    public View showEmpty(int emptyImage) {
        return showEmpty("", emptyImage);
    }

    public View showEmpty(String text, int emptyImage) {
        if (mEmptyView == null) {
            ...
            mEmptyView.setOnClickListener(view -> {
                if (mEmptyClickListener != null) {
                    showLoading();
                    mEmptyView.postDelayed(() -> mEmptyClickListener.onEmptyClick(), 200);
                }
            });
        }
    ...
    public View showRetry(String text) {
        return showRetry(text, -1);
    }

    public View showRetry(int errorImage) {
        return showRetry("", errorImage);
    }

    public View showRetry(String text, int errorImage) {
        if (mRetryView == null) {
            mRetryView = inflate(mRetryResource);

            if (errorImage != -1) {
                ImageView ivError = mRetryView.findViewById(R.id.iv_error);
                ivError.setImageResource(errorImage);
            }

            if (StringUtils.isNotEmpty(text)) {
                TextView tvError = mRetryView.findViewById(R.id.tv_error);
                tvError.setText(text);
            }
            mRetryView.setOnClickListener(v -> {
                if (mRetryClickListener != null) {
                    showLoading();
                    mRetryView.postDelayed(() -> mRetryClickListener.onRetryClick(), 200);
                }
            });
        }
    ...
    public void showLoading(String titleTxt) {
        if (null == progress) {
            progress = new LoadingView(this.context);
            progress.setTitleTxt(titleTxt);
        }
        progress.show();
    }
